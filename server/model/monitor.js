...
class Monitor extends BeanModel {
        ...
        const beat = async () => {
            ...
            try {
                if (this.type === "http" || this.type === "keyword") {
                    ...
                    // Type: Push
                    const time = R.isoDateTime(
                        dayjs.utc().subtract(this.interval, "second")
                    );

                    let heartbeatCount = await R.count(
                        "heartbeat",
                        " monitor_id = ? AND time > ? ",
                        [this.id, time]
                    );
                    ...
                    if (heartbeatCount <= 0) {
                        ...
                        previousBeat = await Monitor.getPreviousHeartbeat(
                            this.id
                        );
    ...
    async updateTlsInfo(checkCertificateResult) {
        let tls_info_bean = await R.findOne(
            "monitor_tls_info",
            "monitor_id = ?",
            [this.id]
        );
